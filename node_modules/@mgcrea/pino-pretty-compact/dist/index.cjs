...
var colorString = (s) => color.green(`'${String(s)}'`);
var colorKeyword = (s) => color.blue(String(s));
var colorJson = (s) => color.green(JSON.stringify(s));
var colorStringArray = (a) => `[ ${a.map(colorString).join(", ")} ]`;
var colorNumber = (n) => color.yellow(String(n));
...
var isValidError = (maybeError) => {
  return isObject(maybeError) && "message" in maybeError && isString(maybeError["message"]) && "stack" in maybeError && isString(maybeError["stack"]);
};
var isSerializedError = (maybeError) => {
  return isObject(maybeError) && "raw" in maybeError && maybeError["raw"] instanceof Error;
};
var serializeError = (maybeError) => {
...
var formatError = (error, level) => {
  const { statusCode = 500, type = String(error["name"]), stack = `${import_os.EOL}    at ???` } = error;
  ...
  const output = [
    color2[isInternalError ? "red" : "yellow"](`${icon}${type} `),
    color2.magenta(Number(statusCode))
  ];
  if (isInternalError) {
...
var build = (options3) => {
  ...
  const messageFormat = (log, messageKey, _leveLabel) => {
    ...
    if (firstErrorKey) {
      ...
      output.push(formatError(serializedError, level), import_os2.EOL);
    } else {
